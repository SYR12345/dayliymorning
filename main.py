...
def get_weather(region):
    ...
    response2 = get(weather_url, headers=headers).json()
    print(response2)
    weather = '白天:' + response2['daily'][0]["textDay"] + '，' + '傍晚:' + \
              response2['daily'][0]["textNight"]
    # 当前温度
    temp = response2['daily'][0]["tempMin"] + u"\N{DEGREE SIGN}" + "C" + '—' + \
           response2['daily'][0]["tempMax"] + u"\N{DEGREE SIGN}" + "C"
    if int(response2['daily'][0]["tempMin"]) <= 25:
        xigua = "天气变凉啦，多穿点衣服哦~"
    else:
        xigua = "今天又是很想你的一天~"
    # 风向
    wind_dir = response2['daily'][0]["windDirDay"]
